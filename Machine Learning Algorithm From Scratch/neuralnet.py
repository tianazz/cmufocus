...
def random_init(shape):
    ...
    W = np.random.uniform(low=-0.1, high=0.1, size=(M, D))
...
class Sigmoid(object):
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        :return: partial derivative of loss with respect to input of sigmoid activation dl/dz
        '''
        # TODO: implement this based on your written answers!
        z_value = self.cache['z']
        return np.multiply(dz, np.multiply(z_value, 1-z_value))
...
class Linear(object):
    def __init__(self, input_size: int, output_size: int, 
        ...
        self.w = np.concatenate((np.zeros((1, self.w.shape[0])), self.w.T), axis=0, dtype=float).T
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        # TODO: implement this based on your written answers!
        if len(x.shape) == 1:
            x_ = np.concatenate(([1], x)).T
        else:
            num_t = x.shape[0]
            #x 500*129
            x_ = np.concatenate((np.ones((1, num_t)), x.T), axis=0, dtype=float).T
        a = np.dot(x_, self.w.T)
        self.cache["x_"] = x_
        return a
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        # the weight matrix!
        self.dw = np.outer(dz, self.cache["x_"])

        return np.dot(self.w[:, 1:].T, dz)
...
class NN(object):
    ...
    def backward(self, y: np.ndarray, y_hat: np.ndarray) -> None:
            ...
            '''
            # TODO: implement this!
            dl_b = d_softmax_cross_entropy(y, y_hat)
            dl_z = self.linear2.backward(dl_b)
            dl_a = self.activation.backward(dl_z)
            self.linear1.backward(dl_a)
    ...
    def step(self):
            ...
            '''
            self.linear2.w = self.linear2.w - self.learning_rate*self.linear2.dw
            self.linear1.w = self.linear1.w - self.learning_rate*self.linear1.dw
...
def test(X: np.ndarray, y: np.ndarray, nn: NN) -> tuple[np.ndarray, float]:
    ...
    for i in range(len(X)):
        ...
        predict[i] = np.argmax(y_hat)
    for i in range(len(y)):
        if predict[i] != y[i]:
            error += 1
    error_rate = error/len(y)
    return predict, error_rate
...
def train(X_tr: np.ndarray, y_tr: np.ndarray, 
    ...
    for e in range(n_epochs):
        x_shuffle_tr, y_shuffle_tr = shuffle(X_tr, y_tr, e)
        for i in range(x_shuffle_tr.shape[0]):
            y_hat = nn.forward(x_shuffle_tr[i])
            nn.backward(y_shuffle_tr[i], y_hat)
            nn.step()
        train_loss = 0
        test_loss = 0
        for i in range(x_shuffle_tr.shape[0]):
            y_hat_tr = nn.forward(x_shuffle_tr[i])
            train_loss += float(cross_entropy(y_shuffle_tr[i], y_hat_tr))
        training_loss.append(train_loss/len(x_shuffle_tr))
        for i in range(X_test.shape[0]):
            y_hat_test = nn.forward(X_test[i])
            test_loss += float(cross_entropy(y_test[i], y_hat_test))
...
if __name__ == "__main__":
    ...
    # TODO: build model
    if init_flag == 2:
        nn = NN(X_tr.shape[1], n_hid, len(labels), zero_init, lr)
    else:
        nn = NN(X_tr.shape[1], n_hid, len(labels), random_init, lr)
